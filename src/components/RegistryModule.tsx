...
export const RegistryModule: React.FC<RegistryModuleProps> = ({
  ...
  const [formData, setFormData] = useState<Partial<RegistryFile>>({
    ...
    courtNumber: 'Court 1',
    magistrate: '',
    advocateName: users?.find(u => u.role === 'Advocate' || u.role === 'Proprietor')?.fullName || 'Adv. James Kamau',
    clerkName: users?.find(u => u.role === 'Clerk')?.fullName || 'Peter Mutua',
    secretaryName: users?.find(u => u.role === 'Secretary')?.fullName || 'Mary Wafula',
  ...
  const handleCreateFile = (e: React.FormEvent) => {
    ...
    const newFile: RegistryFile = {
      ...
      clerkName: formData.clerkName || 'Peter Mutua',
      secretaryName: formData.secretaryName || 'Mary Wafula',
